    ...
    async sendHex(data) {
        ...
        if (Array.isArray(data)) {            
            for (let i=0; i < data.length; i++) {                
                if (typeof data[i] !== 'number' || !Number.isInteger(data[i]) || data[i] < 0 || 255 < data[i]) {
                    console.log("Array contained a value that wasn\'t an integer, or outside of 0 to 255");
            ...
            if (this.port.writable) {
                ...
                const writer = this.port.writable.getWriter();        
                // send it, then release the writer:                
                writer.write(buffer).then(writer.releaseLock());
            }
        }        
    }
